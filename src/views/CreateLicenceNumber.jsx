...
import { updateLicenceNumber, updateNik } from '../actions/RegisterActions';

const CreateLicenceNumber = (props) => {
    const navigation = useNavigation();
    const [licence_number, setLicenceNumber] = useState('');
    const [nik_number, setNikNumber] = useState('');
    let alt = useRef(
    ...
    const check_valid = () => {
        if (licence_number && nik_number) {
            ...
            alt({
                ...
                message: 'Please enter your nik and licence number',
            });
        }
    }

    const navigate = () => {
        console.log('navigate to birth#1')
        props.updateLicenceNumber(licence_number);
        props.updateNik(nik_number)
        console.log('navigate to birth #2')
        navigation.navigate('CreateDateOfBirth', {});
    }

    useEffect(() => {
        console.log(`sim,nik ${licence_number} ${nik_number}`)
    }, [licence_number, nik_number])
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>Masukan No SIM & NIK</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>No SIM dan NIK diperlukan</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
                    ...
                    {/* <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                        <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>Masukan NIK</Text>
                        <View style={{ margin: 5 }} />
                        <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>NIK diperlukan</Text>
                        <View style={{ margin: 20 }} />
                    </View> */}
...
const mapDispatchToProps = (dispatch) => ({
    ...
    updateNik: (data) => dispatch(updateNik(data))
});

function mapStateToProps(state) {
    return {
        licence_number: state.register.licence_number,
        nik: state.register.nik
    };
}

export default connect(mapStateToProps, mapDispatchToProps)(CreateLicenceNumber);