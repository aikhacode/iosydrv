...
import { normal, bold, my_bookings, api_url, img_url, loader, no_data_loader, cancel, f_s, f_xs, f_tiny, f_xl, clickable_icon } from '../config/Constants';
...
const Bookings = (props) => {
    ...
    useEffect(() => { console.log(filter, loading) }, [filter, loading])
    ...
    const ListItem = React.memo(
        ({ item, viewableItems }) => {
            ...
            const getTotalFare = (item) => {
                return parseFloat(item.total) + parseFloat(item.tol) + parseFloat(item.parkir)
            }
            return (
                // <Animated.View style={[
                //     {
                ...
                //     rStyle,
                // ]}>
                <TouchableOpacity activeOpacity={1} onPress={navigate.bind(this, item.id, item.status_name)} style={{ alignItems: 'center', borderRadius: 10, padding: 5 }}>
                    <DropShadow
                        style={{
                            width: '95%',
                            marginBottom: 0,
                    ...
                    >
                        <View style={{ flexDirection: 'row', flex: 1, backgroundColor: 'white', padding: 15, borderRadius: 10, borderColor: 'gray', borderWidth: 1 }}>



                            {/* <View style={{ width: '23%', alignItems: 'flex-start', justifyContent: 'center' }}>
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_xs, fontFamily: bold }}>{item.driver_name}</Text>
                                <View style={{ margin: 2 }} />
                            ...
                            <View style={{ width: '25%', alignItems: 'center', justifyContent: 'center' }}>
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_xs, fontFamily: normal }}>{Moment(item.pickup_date).format("DD-MMM-YYYY")}</Text>
                                <View style={{ margin: 3 }} />
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_xs, fontFamily: bold }}>{Moment(item.pickup_date).format("hh:mm a")}</Text>
                            </View>
                            <View style={{ width: '25%', alignItems: 'center', justifyContent: 'center' }}>
