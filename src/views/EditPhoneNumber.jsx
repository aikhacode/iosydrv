...
const EditPhoneNumber = (props) => {
  ...
  const check_valid = () => {
    if (phone.current.getPickerData() != '') {
      call_update_kyc();
    } else {
      alt({
  ...
  const call_update_kyc = () => {
    console.log('data phone', phone.current.state.value)
    setLoading(true);
    axios({
      method: 'post',
      url: api_url + profile_confirm,
      data: { driver_id: global.id, field: 'phone_with_code', data: phone.current.state.value }
    })
      .then(async response => {
        ...
        alt({
          ...
          title: 'Successfully sent phone number',
          message: 'Your phone number has been sent. Please wait admin confirmation',
        });
  ...
  return (
    <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
      ...
      <View style={{ alignItems: 'center', justifyContent: 'center' }}>
        <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>Masukan nomer HP</Text>
        <View style={{ margin: 5 }} />
        <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>Nomer HP aktif kamu</Text>
        <View style={{ margin: 20 }} />
        <View style={{ width: '80%' }}>
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <View style={{ width: '90%', }}>
              ...
              /> */}
              <ReactNativePhoneInput style={{ borderBottomColor: colors.theme_bg_two }}
