...
import { normal, bold, regular, profile_update, api_url, btn_loader, f_xl, f_xs, f_m, profile_confirm } from '../config/Constants';
...
const EditFirstName = (props) => {
    ...
    const call_profile_update = () => {
        ...
        axios({
            ...
            data: { driver_id: global.id, field: 'first_name', data: first_name }
        })
            .then(async response => {
                console.log('profile_confirm firstname', response.data)
                setLoading(false);
                alt({
                    type: DropdownAlertType.Success,
                    title: 'Successfully sent..',
                    message: 'Your first name has been sent. Please wait for admin approval',
                });
                setTimeout(() => {
                    go_back();
                    // alt.current.close();
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>Masukan nama depan</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>Nama depan kamu</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
            ...
            //     children: <NotificationAndroid appName="Gama Driver" contentText="Please wait for admin approval.."
            //         contentTitle="Profile Update"
            //     />,
            // }}
            />
        </SafeAreaView>
