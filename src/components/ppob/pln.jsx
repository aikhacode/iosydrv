...
const OrderPart = ({ order }) => {
    ...
    return (
        <View style={{ width: '100%', height: 'auto', backgroundColor: 'white', borderRadius: 10, marginBottom: 10, padding: 10 }}>
            ...
            <OrderPartItem item={{ title: 'Nama: ', type: "text", description: order.data.results.nama }} />
            <OrderPartItem item={{ title: 'Tarif daya: ', type: "text", description: order.data.results.tarif_daya }} />
            <OrderPartItem item={{ title: 'Jumlah kwh: ', type: "text", description: order.data.results.jumlah_kwh }} />
            <OrderPartItem item={{ title: 'Token: ', type: "text", description: order.data.results.token }} />
...
const PLNComponent = () => {
    ...
    const [amountNominal, setAmountNominal] = useState(0);
    ...
    const PaymentSuccess = (nominal) => {
        setCanView_({ payment: false, order: false, listing: false })
        console.log('paysucces', amountPay, phoneNumber, nominal)
    ...
    const handlePhoneNumberChange = (text) => {
        if (text.length < 11) {
            setError('Phone number must be at least 11 digits or more');
    ...
    const detectOperator = (number) => {
        for (const [key, prefixes] of Object.entries(operatorPrefixes)) {
    ...
    const call_operator = (operator) => {
        ...
        axios({
            ...
            data: { operator: 'PLN', method: 'get', group: 'PLN' }, // 'PULSA' | 'DATA'
            // headers: {
        ...
        if (boleh)
            axios({
                ...
                if (response.status == 200) {
                    console.log('handlebuy data', response.data)

                    Alert.alert('Confirm Action', // Title
                        `\nPLN Token \nNama: ${response.data.results.nama} \nTarif daya: ${response.data.results.tarif_daya} \nLanjut membeli?`, // Message
                        [
                            {
                                text: 'No',
                                onPress: () => console.log('No Pressed'),
                                style: 'cancel', // Optional: marks this as the cancel button
                            },
                            {
                                text: 'Yes',
                                onPress: () => {
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={[styles.header]}>
                ...
                <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: bold }}>Gama PPOB | PLN Token </Text>
                </View>
            </View>
            <View style={{ flex: 1, padding: 15 }} >
                <View style={styles.bodyContent}>
                    <View style={{ flexDirection: 'row', width: '100%' }}>
                        ...
                        {/* <Image source={ppob_images.pln} style={{ width: '20%', height: '100%' }} /> */}
                    </View>
                    ...
                    {canView.listing ? <FlatList
                        data={pulsaProducts}
                        keyExtractor={(item) => Math.random().toString(36).substring(2, 15)}
                        renderItem={renderProduct}
                        numColumns={2} /> : ""}
                    {canView.payment && <SCPayment amount={amountPay} phone={phoneNumber} onSuccess={() => PaymentSuccess(amountNominal)} />}
